...
void runLocalStrangeJets()
{
  ...
  if (IsFd){ 
    ...
    if(IsJet){
      ...
      if(PCL){ana->AnaMakeFdR(hPtXiPCL, hPtXiNegKine, 2*0.75*TMath::TwoPi()*0.48); }
      if(PCU){ana->AnaMakeFdR(hPtXiPCU, hPtXiNegKine, 2*0.75*TMath::TwoPi()*0.48); }
