...
void UEUncer_wOC(const TString sType = "Xi"){
  
  TString sPath = Form("/home/cuipengyao/study/pp/Local/ThisAna/JE/%s", sType.Data()); 
  TString sOCPath = Form("/home/cuipengyao/study/pp/Local/ThisAna/Uncertainty/JE/UE/OC08"); 
  if(sType == "Omega") sPath = Form("/home/cuipengyao/study/pp/Local/ThisAna/JE_OmegaBins/%s", sType.Data()); 
  if(sType == "Omega") sOCPath = Form("/home/cuipengyao/study/pp/Local/ThisAna/Uncertainty/JE_Omega/UE/OC08"); 
  ...
  hUE1 = (TH1D*)GetTH1D(sOCPath, sFile, OCList, sHist);
  ...
  for(Int_t i = 1; i<=h1->GetNbinsX(); i++){
    ...
    h2->SetBinContent(i, TMath::Abs(V3-V2)/2.); 
    h2->SetBinError(i, TMath::Sqrt(E3*E3 + E2*E2)/2.); 
  ...
  TList *l = new TList();
  l->Add(hUMax);
  l->Write(Form("%s_UESub_wOC",sType.Data()), TObject::kSingleKey);
  f->Close();
