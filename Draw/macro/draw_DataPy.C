...
void draw_DataPy(const TString sType = "Lambda_sum"){
  ...
  Double_t YMax = 1.5; if(sType == "Xi") YMax = 0.2; if(sType == "Omega") YMax = 0.05;
  ...
  DrawHisto(h1, cLine[1], sMark[1], "same"); DrawGraph(g1, cLine[1], "E2"); DrawGraph(g01, cLine[1], "C"); leg->AddEntry(h1, "JE");
  DrawHisto(h2, cLine[2], sMark[2], "same"); DrawGraph(g2, cLine[2], "E2"); DrawGraph(g02, cLine[2], "C"); leg->AddEntry(h2, "UE");
  ...
  //can->SaveAs(Form("./figure/ThisAna/%s_toKRatio_DataPy.eps", sType.Data()));
  CanvasEnd(can);
