...
void RawPtSpect(TString sType = "Kshort"){
  ...
  //if(IsIncl == 1) sPath = "/home/cuipengyao/study/pp/Local/ThisAna/Inclusive";
  ...
  if(IsIncl){
    ...
    auto hIncl = (TH1D*)GetTH1D("/home/cuipengyao/study/pp/Local/ThisAna/Inclusive", sFile.Data(), sInclList.Data(), sHist.Data());
    hIncl->SetName(Form("hIncl")); 
    hEvent = (TH1D*)GetTH1D("/home/cuipengyao/study/pp/Local/ThisAna/Inclusive", sFile.Data(), sInclList.Data(), sEvent.Data());
    hEvent->SetName("hEvent");
    hIncl->SetName(Form("hIncl")); 
    hIncl->Scale(1./(2.*TMath::Pi()*2.*0.75*(hEvent->GetEntries())));
    NormHistBinWidth(hIncl);
    //hIncl->GetYaxis()->SetRangeUser(0.1*(hIncl->GetMinimum()), 10*(hIncl->GetMaximum())); 
    hIncl->GetYaxis()->SetRangeUser(1e-7, 80.); 
    ...
    //TFile *f = TFile::Open("./result/PtSpect_Compare2LF.root", "UPDATE");
    //f->cd(); hIncl->Write(Form("hPtIncl_%s", sType.Data()), TObject::kSingleKey); f->Close();
  }
  
  if(IsJC){
    TString sJCList = Form("%s_%s_Default_JC", sList.Data(), sType.Data()); 
    hJC = (TH1D*)GetTH1D(sPath.Data(), sFile.Data(), sJCList.Data(), sHist.Data());
    hEvent = (TH1D*)GetTH1D(sPath.Data(), sFile.Data(), sJCList.Data(), sEvent.Data());
    hEvent->SetName("hEvent_JC");
    cout<<hEvent->GetEntries()<<endl;
    hJC->Scale(1./(2.*TMath::Pi()*2.*0.75*(hEvent->GetEntries())*0.06));
    ...
    //f->cd(); hJC->Write(Form("hPtJC_%s", sType.Data()), TObject::kSingleKey); f->Close();
  } 
  ...
  if(IsPC){
    ...
    hEvent = (TH1D*)GetTH1D(sPath.Data(), sFile.Data(), sPCLList.Data(), sEvent.Data());
    hEvent->SetName("hEvent_PC");
    hPC->Scale(1./(2.*TMath::Pi()*2.*0.75*(hEvent->GetEntries())*0.06*2));
    ...
    //TFile *f = TFile::Open("./result/PtSpect_ThisAna.root", "UPDATE");
    //f->cd(); hPC->Write(Form("hPtPC_%s", sType.Data()), TObject::kSingleKey); f->Close();
  }
  ...
  if(IsOC){
    ...
    //TFile *f = TFile::Open("./result/PtSpect_ThisAna.root", "UPDATE");
    //f->cd(); hOC->Write(Form("hPtOC_%s", sType.Data()), TObject::kSingleKey); f->Close();
  } 
  ...
  //  //TFile *f = TFile::Open("./result/PtSpect_ThisAna.root", "UPDATE");
  //  //f->cd(); hJE->Write(Form("hPtJE_%s", sType.Data()), TObject::kSingleKey); f->Close();
  //} 
  ...
  if(IsNJ){
    ...
    //TFile *f = TFile::Open("./result/PtSpect_ThisAna.root", "UPDATE");
    //f->cd(); hNJ->Write(Form("hPtNJ_%s", sType.Data()), TObject::kSingleKey); f->Close();
  } 
  ...
  tex->DrawLatex(0.15, 0.3, Form("K^{0}_{S} raw yields"));
  tex->DrawLatex(0.15, 0.2, "|#it{#eta}_{K^{0}_{S}}| < 0.75, #Delta#it{R}(K^{0}_{S}, jet) < 0.4");
  leg->Draw();
  can->SaveAs(Form("./figure/%s_wo_EffiCorr.eps", sType.Data()));
  can->SaveAs(Form("./figure/%s_wo_EffiCorr.png", sType.Data()));
  CanvasEnd(can);
