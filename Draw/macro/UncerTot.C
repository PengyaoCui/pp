...
void UncerTot(const TString sType = "Kshort"){
  
  auto sPath = "./result"; 
  auto sCFile = "PtSpect_ThisAna.root";
  
  auto sCInclHist = Form("hPtIncl_%s", sType.Data());
  auto sEInclFile = "InclUncertainty.root";
  ...
  if(sType == "Lambda" || sType == "AntiLa") hJC1->Add(hUE1);
  ...
  auto hJErrContJ = (TH1D*)hJE2->Clone("hJErrContJ"); hJErrContJ->Reset();//Jet pT Scale Uncertainty contribution to JE spectrum 
  ...
  //TFile *f = TFile::Open("./result/UncertaintyTot.root", "UPDATE");
  //TList *l = new TList();
  //l->Add(hIErr);
  //l->Add(hJErr);
  //l->Add(hUErr);
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JCUncertCont_Max-Min.eps", sType.Data()));
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JCUncertCont_Max-Min.pdf", sType.Data()));
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JCUncertCont_Max-Min.png", sType.Data()));
