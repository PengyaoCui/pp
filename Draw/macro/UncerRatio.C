...
void UncerRatio(const TString sType = "Xi"){
  ...
  leg1->Draw();
  gStyle->SetOptStat("");
  can1->SaveAs(Form("./figure/CheckUEsubUncer/%s_toK_Uncert_NJ-PC_OC-PC.eps", sType.Data()));
  can1->SaveAs(Form("./figure/CheckUEsubUncer/%s_toK_Uncert_NJ-PC_OC-PC.pdf", sType.Data()));
  can1->SaveAs(Form("./figure/CheckUEsubUncer/%s_toK_Uncert_NJ-PC_OC-PC.png", sType.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can1);
  return;
}
