...
void OLRatioUncer(const TString CentMin = "0", const TString CentMax = "100"){
  ...
  auto CListP2 = "listPC02";
  auto CListP6 = "listPC06";
 
  auto hInR = (TH1D*)GetTH1D(sPath, CFile, CListIn, Form("hOLRatio")); hInR->SetName("hInR"); ;
  auto hJCR = (TH1D*)GetTH1D(sPath, CFile, CListJC, Form("hOLRatio")); hJCR->SetName("hJCR"); ;
  auto hPCR = (TH1D*)GetTH1D(sPath, CFile, CListPC, Form("hOLRatio")); hPCR->SetName("hUER"); ;
  auto hJER = (TH1D*)GetTH1D(sPath, CFile, CListJE, Form("hOLRatio")); hJER->SetName("hJER"); ;


  auto sListD =  Form("Lambda_sum");
  auto sList =  Form("Omega"); 
  //---------------------------InclUncer
  ...
  heUEK = RebinToOmega(heUEK, heUER);  
  ...
  heJEi = RebinToOmega(heJEi, heJEI);  
  ...
  hP6K = (TH1D*)RebinToOmega(hP6K, hP6R);
  hP2K = (TH1D*)RebinToOmega(hP2K, hP6R);
  ...
  hJCP6K = (TH1D*)RebinToOmega(hJCP6K, hP6R);
  hJCP2K = (TH1D*)RebinToOmega(hJCP2K, hP6R);

  hJCP6->Add(hP6, -1.); hJCP2->Add(hP2, -1.);
  hJCP6K->Add(hP6K, -1.); hJCP2K->Add(hP2K, -1.);

  hP6R->Divide(hJCP6, hJCP6K, 1., 1.);
  hP2R->Divide(hJCP2, hJCP2K, 1., 1.);
  ...
  TH1D* hJPt0 = nullptr;
  TH1D* hJPt1 = nullptr;
  hJPt0 =(TH1D*)GetTH1D(sPath, sFile3, Form("Omega_JetPt"), "hist_0"); hJPt0->SetName("hJPt0");
  hJPt1 =(TH1D*)GetTH1D(sPath, sFile3, Form("Omega_JetPt"), "hist_1"); hJPt1->SetName("hJPt1");
  
  hJPt0K = (TH1D*)RebinToOmega(hJPt0K, hJPt0);
  hJPt1K = (TH1D*)RebinToOmega(hJPt1K, hJPt1);
  ...
  heJCi = RebinToOmega(heJCi, heJCI);
  ...
  l->Write(Form("Omega_toLRatio"), TObject::kSingleKey);
  f->Close();
  
  ofstream OutFile;
  OutFile.open(Form("./dat/OLRatioUncer.dat"));
  OutFile<<left<<setw(15)<<"OLRatio"<<endl;
  ...
  tex->DrawLatex(0.15, 0.65, Form("#Omega/#Lambda"));
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/OLRatio_JCUncert.pdf"));
  can->SaveAs(Form("./figure/Uncert/OLRatio_JCUncert.eps"));
