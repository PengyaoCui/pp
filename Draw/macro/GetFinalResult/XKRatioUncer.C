...
void XKRatioUncer(){
  ...
  auto hInR = (TH1D*)GetTH1D(sPath, CFile, CListIn, Form("hXKRatio")); hInR->SetName("hInR"); ;
  auto hJCR = (TH1D*)GetTH1D(sPath, CFile, CListJC, Form("hXKRatio")); hJCR->SetName("hJCR"); ;
  auto hPCR = (TH1D*)GetTH1D(sPath, CFile, CListPC, Form("hXKRatio")); hPCR->SetName("hUER"); ;
  auto hJER = (TH1D*)GetTH1D(sPath, CFile, CListJE, Form("hXKRatio")); hJER->SetName("hJER"); ;
  ...
  l->Write(Form("Xi_toKRatio"), TObject::kSingleKey);
  f->Close();

  ofstream OutFile;
  OutFile.open(Form("./dat/XKRatioUncer.dat"));
  OutFile<<left<<setw(15)<<"XKRatio"<<endl;
  ...
  tex->DrawLatex(0.15, 0.65, Form("#Xi/K^{0}_{S}"));
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/XKRatio_JCUncert.pdf"));
  can->SaveAs(Form("./figure/Uncert/XKRatio_JCUncert.eps"));
