...
void StrangeRecUncer(const TString sType = "Lambda", const TString sUncertType = "CompetMass"){
  ...
  TH1D* hRatio = nullptr;
  TH1D* hU[nhist];
  h2 = (TH1D*)GetTH1D(Form("/home/cuipengyao/study/pp/Local/ThisAna/Incl"), sFile.Data(), List.Data(), sHist.Data());
  ...
  TFile *f = TFile::Open("./result/StrangeRecUncer.root", "UPDATE");
  //TFile *f = TFile::Open("./result/TestSource.root", "UPDATE");
  TList *l = new TList();
  for (Int_t i = 0; i< nHist; ++i){
    TString sMyPath = Form("%s/%s", sPath.Data(), Cut[i].Data()); 
    if(i == 0 && sType == "Omega" && sUncertType == "TPCPID") sMyPath = "/home/cuipengyao/study/pp/Local/ThisAna/Incl"; 
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  if(IsSaveCan) can->SaveAs(Form("./figure/Uncert/%s_%s.pdf", sType.Data(), sUncertType.Data()));
  CanvasEnd(can);
  return;
}
