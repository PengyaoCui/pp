...
void OKRatioUncer(){
  ...
  auto hInR = (TH1D*)GetTH1D(sPath, CFile, CListIn, Form("hOKRatio")); hInR->SetName("hInR"); ;
  auto hJCR = (TH1D*)GetTH1D(sPath, CFile, CListJC, Form("hOKRatio")); hJCR->SetName("hJCR"); ;
  auto hPCR = (TH1D*)GetTH1D(sPath, CFile, CListPC, Form("hOKRatio")); hPCR->SetName("hUER"); ;
  auto hJER = (TH1D*)GetTH1D(sPath, CFile, CListJE, Form("hOKRatio")); hJER->SetName("hJER"); ;
  ...
  l->Write(Form("Omega_toKRatio"), TObject::kSingleKey);
  f->Close();

  ofstream OutFile;
  OutFile.open(Form("./dat/OKRatioUncer.dat"));
  OutFile<<left<<setw(15)<<"OKRatio"<<endl;
  ...
  tex->DrawLatex(0.15, 0.65, Form("#Omega/K^{0}_{S}"));
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/OKRatio_JCUncert.pdf"));
  can->SaveAs(Form("./figure/Uncert/OKRatio_JCUncert.eps"));
