...
void LKRatio(){
  ...
  TString sL[] = {"hPtIncl_Lambda", "hPtPC_Lambda", "hPtJC_Lambda"};
  TString sA[] = {"hPtIncl_AntiLa", "hPtPC_AntiLa", "hPtJC_AntiLa"};
 

  TString sLeg[] = {"Incl", "PC", "JE"};

  auto sLatex("Inclusive #Lambda to K^{0}_{S} ratio");
  Double_t XMin = 0.6;
  Double_t XMax = 12;
  //-----------------------------------
  ...
  TFile *f = TFile::Open("./result/LKRatio.root", "UPDATE");
  can = MakeCanvas("can");
  ...
  gStyle->SetOptStat("");
  CanvasEnd(can);
  //can->SaveAs(Form("./figure/LambdatoKshortRatio_PC.eps"));
  return;
}
