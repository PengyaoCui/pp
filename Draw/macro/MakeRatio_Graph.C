...
void MakeRatio_Graph(const TString CentMin = "20", const TString CentMax = "40", const Int_t i = 5, const Int_t j = 4 ){
                     
  
  
  TString sPathNu[] = {"./result/"};
  TString sFileNu[] = {"PtSpect_Compare2LF"};
  TString sListNu[] = {""};
  TString sHistNu[] = {Form("hPtIncl_Kshort_%s%s", CentMin.Data(), CentMax.Data())};
  TString sHistNuPos[] = {Form("hPtIncl_Kshort_%s%s", CentMin.Data(), CentMax.Data())};
  ...
  auto sLatex(Form("Inclusive #Oemga + #bar{#Omega} spectra (%s-%s%%)", CentMin.Data(), CentMax.Data()));
  Double_t YMin = 1e-12;
  Double_t YMax = 1e12;
  Double_t XMin = 0.8;
  Double_t XMax = 5.;
  ...
  gStyle->SetOptStat("");
  CanvasEnd(can);
  //can->SaveAs(Form("./figure/Kshort_ComparetoLF_%s%s.eps", CentMin.Data(), CentMax.Data()));
  return;
}
