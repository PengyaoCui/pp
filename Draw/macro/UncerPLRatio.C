...
void UncerPLRatio(const TString sType = "Xi"){
  
  const TString sK = "Lambda_sum";
  auto sPath = "./result"; 
  ...
  //===========================UESub
  auto hNJK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_Lambda")); hNJK->SetName("hNJK");
  auto hOCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_Lambda")); hOCK->SetName("hOCK");
  auto hPCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_Lambda")); hPCK->SetName("hPCK");

  auto hNJA = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_AntiLa")); hNJA->SetName("hNJA");
  auto hOCA = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_AntiLa")); hOCA->SetName("hOCA");
  auto hPCA = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_AntiLa")); hPCA->SetName("hPCA");
  hNJK->Add(hNJA); hNJK->SetName("hNJK");
  hOCK->Add(hOCA); hOCK->SetName("hOCK");
  hPCK->Add(hPCA); hPCK->SetName("hPCK");
  ...
  auto hJPtK00 =(TH1D*)GetTH1D(sPath, sFile3, Form("AntiLa_JetPt"), "hist_0"); hJPtK00->SetName("hJPtK00");
  auto hJPtK11 =(TH1D*)GetTH1D(sPath, sFile3, Form("AntiLa_JetPt"), "hist_1"); hJPtK11->SetName("hJPtKK11");
  hJPt0K->Add(hJPtK00);hJPt0K->SetName("hJPt0K");  hJPt1K->Add(hJPtK11);hJPt1K->SetName("hJPt1K");
  ...
  l->Write(Form("%s_toLRatio",sType.Data()), TObject::kSingleKey);
  f->Close();
  ...
  if(sType == "Omega"){XMax = 5.; YMax = 0.2;}
  ...
  hInclR->GetYaxis()->SetTitle(Form("#%s/#Lambda", sType.Data()));
  ...
  tex->DrawLatex(0.15, 0.63, Form("#frac{#%s + #bar{#%s}}{#Lambda + #bar{#Lambda}}", sType.Data(), sType.Data()));
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/ThisAna/%s_toLRatio.eps", sType.Data()));
  ...
  leg1->Draw();
  gStyle->SetOptStat("");
  can1->SaveAs(Form("./figure/Uncert/%s_toL_Uncert.eps", sType.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can1);
  return;
}
