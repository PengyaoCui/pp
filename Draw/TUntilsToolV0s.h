...
{
  TH1D *hRatioV = (TH1D*)hKshortErr->Clone("hRatioV"); hRatioV->Reset();

  for (Int_t k=1; k<=hRatioV->GetNbinsX(); k++) {
    Double_t dVarLambda = hLambdaVar->GetBinContent(k);
    Double_t dVarAntiLa = hAntiLaVar->GetBinContent(k);

    Double_t dErrLambda = hLambdaErr->GetBinContent(k) * dVarLambda;
    Double_t dErrAntiLa = hAntiLaErr->GetBinContent(k) * dVarAntiLa;

    Double_t dVarL = dVarLambda + dVarAntiLa; if (TMath::Abs(dVarL)<1e-12) dVarL = 1e-12;
    Double_t dErrL = TMath::Sqrt(dErrLambda*dErrLambda + dErrAntiLa*dErrAntiLa) / dVarL;

    Double_t dErrK = hKshortErr->GetBinContent(k);
...
TH1D *GetNormJetSpecturm(){

  TString sFile = Form("AnalysisOutputs_JetSparse.root");
  TString sList = Form("listSparseJet");
  TFile   *file = TFile::Open(sFile.Data(), "READ"); if (!file->IsOpen()) return 0x0;
  TList   *list = (TList*)file->Get(sList.Data());   if (!list) return 0x0;
  file->Close();
  TH1D     *hPt    = (TH1D*)list->FindObject("hPt"); 

  TH1D *hPts=hPt->Clone("hPts");
  file =TFile::Open("AnalysisResults.root","READ") ;if (!file->IsOpen()) return 0x0;
  list = (TList*)file->Get("listEmcalJet");   if (!list) return 0x0;

  TH1D  *hEvent= (TH1D*)list->FindObject("fHistEventCount"); 
  hPts->Scale(1./hEvent->GetBinContent(1));
  return hPts;
}
...
//------------------------------------------------------ZYH-------------------------------------------------
TH1D *GetV0NormArJEPCM(const TString   sPathMB,
                        const TString   sPathJE,
                        const TString   sV0,
                        const TString   sCut,
                        const TString   sPtJ,
...
if ((sPathMB.IsNull()) ||
  ...
  if ((nPtBin<=0) || (!dPtBin)) return 0x0;
  TString sFileM = Form("%s/AnalysisOutputs_LoopJetRD.root",sPathJE.Data());
  TString sListL = Form("listLoopJetRD_%s_%s_%s_%s",sV0.Data(),sCut.Data(),sPtJ.Data(),sOpt.Data());
  ...
  TH1D *hV0L = (TH1D*)CalcV0sCorrJE(sPathMB,sPathJE,sV0,sCut,sPtJ,sOpt,nPtBin,dPtBin,dEtaMin,dEtaMax); hV0L->SetName("hV0L");

  Double_t dEvL = hEvL->Integral();

  Double_t dArL = hArL->GetBinContent(3) / hArL->GetBinContent(1);

  Double_t dArM = TMath::TwoPi() * (dEtaMax - dEtaMin) * (dArL);
  ...
  for (Int_t k=1; k<=hV0M->GetNbinsX(); k++) {
    ...
    hV0M->SetBinContent(k, (dValL)/dArM);
    hV0M->SetBinError(k, TMath::Sqrt(dErrL*dErrL)/dArM);
  }

  return hV0M;
}
//____________________ZYH______________________________________________________
TH1D *GetRawV0sSBRatio(const TString   sPath,
                       const TString   sV0,
                       const TString   sCut,
                       const Int_t     type=1, 
                       const Double_t  dEtaMin=-0.75,
                       const Double_t  dEtaMax= 0.75)
{
  ...
  if(type==1){
    sFile = Form("%s/AnalysisOutputs_Loop2ndRD.root",sPath.Data());
    sList = Form("listLoop2ndRD_%s_%s",sV0.Data(),sCut.Data());
  }else if(type==2){
    sFile = Form("%s/AnalysisOutputs_FitCbinRD.root",sPath.Data());
    sList = Form("listFitCbinRD_%s_%s",sV0.Data(),sCut.Data());
  }else(
    cout<<"wrong type number"<<endl;return 0x0;
  )
  ...
  if(type==1){
    hS=(TH1D*)list->FindObject("hCountInW");
    hB=(TH1D*)list->FindObject("hCountInB");
  }else if(type==2){
    hS=(TH1D*)list->FindObject("hS");
    hB=(TH1D*)list->FindObject("hB");
  }else{
    cout<<"wrong type"<<endl; return 0x0;
  }
  TH1D *hRatio=hS->Clone();
  for(int i=1;i<hB->GetXaxis()->GetNbins();i++){
     if(hB->GetBinContent(i)<1e-12) hB->SetBinContent(i,1e-12);   
  }
...
TH1D *GetJetV0sSBRatio(const TString   sPath,
                       const TString   sPathJE,
                       const TString   sV0,
                       const TString   sCut,
                       const TString   sPtJ,
                       ...
                       const Double_t  dEtaMax= 0.75)
{
  ...
  if(type==1){
    sFile = Form("%s/AnalysisOutputs_LoopJetRD.root",sPathJE.Data());
    sList = Form("listLoopJetRD_%s_%s_%s_%s",sV0.Data(),sCut.Data(),sPtJ.Data(),sOpt.Data());
  }else if(type==2){
    sFile = Form("%s/AnalysisOutputs_FitCbinJE.root",sPath.Data());
...
//_ZYH___________________________________________________________________________

TH1D  *GetNormRawV0(const TString   sPath,
                    const TString   sV0,
                    const TString   sCut,
                    const Double_t  dEtaMin=-0.75,
                    const Double_t  dEtaMax= 0.75)
{
  if ((sPath.IsNull()) || (sV0.IsNull()) || (sCut.IsNull())) return 0x0;
  TString sFile = Form("%s/AnalysisOutputs_Loop2ndRD.root",sPath.Data());
  TString sList = Form("listLoop2ndRD_%s_%s",sV0.Data(),sCut.Data());
  TFile *file = TFile::Open(sFile.Data(), "READ"); if (!file->IsOpen()) return 0x0;
  TList *list = (TList*)file->Get(sList.Data());   if (!list) return 0x0; file->Close();
  ...
  TH1D *hV0 =(TH1D*)list->FindObject("hS");
...
//_ZYH___________________________________________________________________________________
TH1D *GetNormIncEffV0(const TString   sPath,
                      const TString   sV0,
                      const TString   sCut,
                      const Double_t  dEtaMin=-0.75,
                      const Double_t  dEtaMax= 0.75)
{
  if ((sPath.IsNull()) || (sV0.IsNull()) || (sCut.IsNull())) return 0x0;
  TString sFile = Form("%s/AnalysisOutputs_Loop1ndRD.root",sPath.Data());
  TString sList = Form("listLoop1ndRD_%s_%s",sV0.Data(),sCut.Data());
  TFile *file = TFile::Open(sFile.Data(), "READ"); if (!file->IsOpen()) return 0x0;
  TList *list = (TList*)file->Get(sList.Data());   if (!list) return 0x0; file->Close();
  ...
  sFile = Form("%s/AnalysisOutputs_EffCorrMC.root",sPath.Data());
  sList = Form("listEffCorrMC_%s_%s",sV0.Data(),sCut.Data());
  file = TFile::Open(sFile.Data(), "READ"); if (!file->IsOpen()) return 0x0;
  list = (TList*)file->Get(sList.Data());   if (!list) return 0x0; file->Close();
  TH1D *hPtEff=(TH1D*)list->FindObject("hPtEffi");


  for (Int_t k=1; k<=hV0->GetNbinsX(); k++) {
    hV0->SetBinContent(k, hV0->GetBinContent(k)/dNorm/hPtEff->GetBinContent(k));
    hV0->SetBinError(k, hV0->GetBinError(k)/dNorm);
  }

  return hV0;
}
//__ZYH_________________________________________________________________________________________
TH1D *GetNormJetV0(const TString   sPath,
                   const TString   sPathJE, 
                   const TString   sV0,
                   const TString   sCut,
                   const TString   sPtJ,
                   ...
                   const Double_t  dEtaMax= 0.75)
{
  ...
  if ((sPath.IsNull()) || (sV0.IsNull()) || (sCut.IsNull())||sPathJE.IsNull()||sPtJ.IsNull()||sOpt.IsNull()) return 0x0;
...
//===============================================================
TH1D *GetJetV0Can( const TString   sPath,
                   const TString   sPathJE,
                   const TString   sV0,
                   const TString   sCut,
                   const TString   sPtJ,
                   ...
                   const Double_t  dEtaMax= 0.75)
{
  ...
  TH2D *hPtInvM = (TH2D*)list->FindObject("hPtInvM");

  cout<<hPtInvM->GetEntries()<<endl;


  TH1D *hV0= hPtInvM->ProjectionX(sV0.Data());
...
//================================================================================
TH2D *GetEtaPhi(const TString   sPath,
                const TString   sPathJE,
                const TString   sV0,
                const TString   sCut,
                const TString   sPtJ,
...
{
  TString sFile = Form("%s/AnalysisOutputs_LoopJetRD.root",sPathJE.Data());
  TString sList = Form("listLoopJetRDDeltaVJ_%s_%s_%s_%s",sV0.Data(),sCut.Data(),sPtJ.Data(),sOpt.Data());
  ...
  list->ls();
  THnSparseD *hn=(THnSparseD*) list->FindObject("hsCor");
  TH2D *hEtaPhi=(TH2D*)hn->Projection(7,8);
  return hEtaPhi;
}
//===============================================================================
TH1D *GetPhi(const TString   sPath,
             const TString   sPathJE,
             const TString   sV0,
             const TString   sCut,
             const TString   sPtJ,
             ...
             const Double_t bin2)
{
  ...
  int from_bin = hn->GetAxis(4)->FindBin(bin1);
  int to_bin   = hn->GetAxis(4)->FindBin(bin2);

  hn->GetAxis(4)->SetRange(from_bin, to_bin);

  TH1D *hPhi =hn->Projection(8);
  return hPhi;
...
//___________________________________________________________________zyh_____
TH1D *GetNormJetEffV0(const TString   sPath,
                   const TString   sPathJE,
                   const TString   sV0,
                   const TString   sCut,
                   const TString   sPtJ,
...
//Double_t dNorm=1; 
  ...
  sFile = Form("%s/AnalysisOutputs_EffCorrJE.root",sPath.Data());
  sList = Form("listEffCorrJE_%s_%s_%s_%s",sV0.Data(),sCut.Data(),sPtJ.Data(),sOpt.Data());
...
cout<<ll<<" " <<lu<<endl;
  ...
  for (Int_t k=1; k<=hPtEtaRecoRD_RB->GetNbinsX(); k++) {
    ...
    //  cout<<" dfv "<<dfv<<" dfe "<<dfe<<" dev "<<dev<<" dee "<<dee<<"====>"<<" ddv "<<ddv<<" dde "<<dde<<endl;  
   } dde  = TMath::Sqrt(dde);

     if(TMath::Abs(dde)>1e15) { dde=0;}
//    cout<<dde<<endl;
...
//_____________________________________________________________________________
Double_t GetEventMB(const TString sPath,
                    const TString sV0,
                    const TString sCut="Default")
{
  if ((sPath.IsNull()) || (sV0.IsNull()) || (sCut.IsNull())) return 0.;
//=============================================================================

  TString sFile = Form("%s/AnalysisOutputs_Loop1stRD.root",sPath.Data());
  TString sList = Form("listLoop1stRD_%s_%s",sV0.Data(),sCut.Data());

  TFile *file = TFile::Open(sFile.Data(), "READ"); if (!file->IsOpen()) return 0.;
  TList *list = (TList*)file->Get(sList.Data());   if (!list) return 0.; file->Close();
//=============================================================================

  TH1D    *hEvent = (TH1D*)list->FindObject("hEvent");
  Double_t dEvent = hEvent->Integral();
...
//=============================================================================
  ...
  if (sOpt.Contains("NJ")) dAr = hAr->GetBinContent(2)/hAr->GetBinContent(1);
/*if (sOpt.Contains("OC")) dAr =       hAr->GetBinContent(2)/hAr->GetBinContent(1);
  if (sOpt.Contains("JC")) dAr = 1. - (hAr->GetBinContent(2)/hAr->GetBinContent(1));
  if (sOpt.Contains("NJ")) dAr =       hAr->GetBinContent(2)/hAr->GetBinContent(1);*/
  Double_t dNorm = hEv->Integral() * TMath::TwoPi() * (dEtaMax - dEtaMin) * dAr;
